...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["project_id"])) {
    ...
    SELECT 
        ...
        SUM(CASE WHEN t.status != 'Completed' AND t.status IS NOT NULL THEN 1 ELSE 0 END) as pending_tasks
    FROM users u
    JOIN project_assignments pa ON u.id = pa.employee_id
    LEFT JOIN task_assignments ta ON u.id = ta.employee_id
    LEFT JOIN tasks t ON ta.task_id = t.id 
    WHERE pa.project_id = ? 
    AND (t.project_id = ? OR t.project_id IS NULL)  -- Ensure tasks are from this project only
...
";
    ...
    while ($row = $result->fetch_assoc()) {
        $employees[] = [
            'name' => $row['name'],
            'total_tasks' => (int) $row['total_tasks'] ?: 0,  // Convert NULL to 0
